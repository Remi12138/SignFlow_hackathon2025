...
function drawLine(
  ...
  x1: number, y1: number, z1: number,
  x2: number, y2: number, z2: number,
  color: number = 0xff0000,  // Red for visibility
  opacity: number = 1,  // Add opacity parameter
  width: number = 0.1  // Add width parameter (controls thickness)
) {
  ...
  const geometry = new THREE.TubeGeometry(path, 8, width, 8, false); // Thick, smooth curve
